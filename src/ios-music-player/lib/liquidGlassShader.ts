// Copy apa adanya dari "liquid-glass-react" (rdev/liquid-glass-react, MIT
// License, Copyright 2025 Max Rovensky) — src/shader-utils.ts.
// Aslinya di-adapt dari https://github.com/shuding/liquid-glass.
// Dipakai buat mode "shader" (Experimental) di panel pengaturan kaca —
// generate displacement map on-the-fly pakai canvas 2D (bukan WebGL,
// meski namanya "shader"), jadi bisa langsung dipakai di sini tanpa
// dependensi tambahan. TIDAK diedit sama sekali dari versi aslinya.
...
function roundedRectSDF(
  ...
  return (
    Math.min(Math.max(qx, qy), 0) + length(Math.max(qx, 0), Math.max(qy, 0)) - radius
  );
}
