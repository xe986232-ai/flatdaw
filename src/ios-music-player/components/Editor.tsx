...
// FLOOR di dalam getLyricsTimeline sendiri, bukan cuma di sini) supaya
// gak ada 2 angka beda yang harus disinkronin manual. Dipakai di
// handleLyricsClipStretchStart & handleCutLyricsClip buat nge-klem
// panjang klip SEBELUM sempat lebih pendek dari titik animasinya bakal
// lebih cepat dari batas ini.
...
export default function Editor({
  ...
  // Hitung "panjang klip minimum yang nyaman" buat SATU layer lirik —
  // dipakai bareng-bareng oleh handleLyricsClipStretchStart (drag tepi
  // kanan) & handleCutLyricsClip (gunting jadi 2 klip), biar KEDUA jalur
  // yang bisa mendekin klip lirik terkena batas yang SAMA. Sebelum ini,
  // cuma jalur drag yang diklem — gunting/cut deket ujung klip masih bisa
  // ngasilin klip sependek MIN_LYRICS_CLIP_DURATION (0.4d) walau teksnya
  // butuh animasi in+out+stagger jauh lebih lama, jadi tetep bisa bikin
  ...
  function handleCutLyricsClip(baseId: string) {
    ...
    // Klem pakai comfortableMin (bukan cuma MIN_LYRICS_CLIP_DURATION
    // mentah) — kedua potongan hasil gunting WARISAN in/out/stagger yang
    // sama persis dari klip asalnya (lihat leftLayer/rightLayer di bawah),
    // jadi masing-masing juga harus tetap >= comfortableMin biar animasi
    // in/out-nya nggak keperas ngebut abis di-gunting (celah yang sama
    // kayak drag-stretch, lihat computeLyricsComfortableMin).
    const comfortableMin = computeLyricsComfortableMin(baseId, eff);
    if (
      cutSec <= eff.startSec + comfortableMin ||
      cutSec >= eff.endSec - comfortableMin
    ) {
      return;
    }
  ...
  return (
    <div className="fx-fade relative flex h-[100dvh] w-screen flex-col overflow-hidden bg-editor-bg font-sans">
      ...
      <Timeline
        ...
        handleTimelineDragMove={handleTimelineDragMove}
        handleTimelineDragEnd={handleTimelineDragEnd}
        DURATION={DURATION}
        TIME_MARKS={TIME_MARKS}
        TRACK_WIDTH={TRACK_WIDTH}
        TIMELINE_CLIP_OFFSET_PX={TIMELINE_CLIP_OFFSET_PX}
        effectivePxPerSec={effectivePxPerSec}
        viewportWidth={viewportWidth}
        timelineScrollRef={timelineScrollRef}
        timelineZoom={timelineZoom}
        MIN_TIMELINE_ZOOM={MIN_TIMELINE_ZOOM}
        MAX_TIMELINE_ZOOM={MAX_TIMELINE_ZOOM}
        TIMELINE_ZOOM_STEP={TIMELINE_ZOOM_STEP}
        zoomTimelineBy={zoomTimelineBy}
        resetTimelineZoom={resetTimelineZoom}
        handleTimelinePinchPointerDown={handleTimelinePinchPointerDown}
        handleTimelinePinchPointerMove={handleTimelinePinchPointerMove}
        handleTimelinePinchPointerUp={handleTimelinePinchPointerUp}
        handlePlayheadPointerDown={handlePlayheadPointerDown}
        trackSelectMode={trackSelectMode}
        selectedTrackBaseIds={selectedTrackBaseIds}
        cancelTrackSelectMode={cancelTrackSelectMode}
        trackGroupMenuOpen={trackGroupMenuOpen}
        setTrackGroupMenuOpen={setTrackGroupMenuOpen}
        canMakeGroupFromSelection={canMakeGroupFromSelection}
        handleMakeGroupFromSelection={handleMakeGroupFromSelection}
        matchingSelectedGroup={matchingSelectedGroup}
        handleUngroupSelection={handleUngroupSelection}
        setTrackSelectMode={setTrackSelectMode}
        toggleRowTrackSelect={toggleRowTrackSelect}
        groupOfBaseId={groupOfBaseId}
        isTextMode={isTextMode}
        lyricsRowDragHint={lyricsRowDragHint}
        allLyricsLayers={allLyricsLayers}
        allTextLayers={allTextLayers}
        lyricsTextEntries={lyricsTextEntries}
        customLyricsLayers={customLyricsLayers}
        autoCompactLyricsRows={autoCompactLyricsRows}
        showAddTextStyles={showAddTextStyles}
        setShowAddTextStyles={setShowAddTextStyles}
        addCustomTextLayer={addCustomTextLayer}
        lyricsBaseIdOf={lyricsBaseIdOf}
        lyricsRowOf={lyricsRowOf}
        getEffectiveLyricsLayer={getEffectiveLyricsLayer}
        handleLyricsRowDragStart={handleLyricsRowDragStart}
        handleLyricsClipDragStart={handleLyricsClipDragStart}
        handleLyricsClipStretchStart={handleLyricsClipStretchStart}
        selectedTextLayerId={selectedTextLayerId}
        setSelectedTextLayerId={setSelectedTextLayerId}
        textValues={textValues}
        setTextToolbarMode={setTextToolbarMode}
        dismissAirplayHint={dismissAirplayHint}
        hasSlotTracks={hasSlotTracks}
        ...
        handleAudioClipDragStart={handleAudioClipDragStart}
        handleAudioClipTrimStart={handleAudioClipTrimStart}
        clampNum={clampNum}
        downsamplePeaks={downsamplePeaks}
        FALLBACK_PEAKS={FALLBACK_PEAKS}
        audioSlotDef={audioSlotDef}
        audioMedia={audioMedia}
        openPicker={openPicker}
        adjustableLayers={adjustableLayers}
        layerOpacity={layerOpacity}
        mediaSlotDef={mediaSlotDef}
      />
